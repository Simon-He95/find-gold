...
function touchmove() {
  const fn = throttle((e) => {
    e.preventDefault()
    const { clientX, clientY } = e.changedTouches[0]
    if (clientX > startX && rightMove()) changeShow()

    if (clientX < startX && leftMove()) changeShow()

    if (clientY > startY && downMove()) changeShow()

    if (clientY < startY && topMove()) changeShow()
  })
  return fn
}
...
<template>
  <div flex="~ gap-5" justify-center items-end m-b-5 max-w-150 ma>
    <div flex="~ gap-4" items-center justify-center>
      <img w-7 src="/img/reset.svg" alt="reset" @click="restart">
